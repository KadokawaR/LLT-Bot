 ...
 * 这个类用来处理骰子相关消息
 */
public class DiceCommandHandler {
    ...
    public static void executeDiceCommandFromGroup(GroupMessageEvent event){
        if(PATTERN_COMMON_COMMAND.matcher(event.getMessage().contentToString()).matches()){
            DiceManager.getCustomDice(captureFromPatternCommon(event.getMessage().contentToString()),1)
                    .broadcastResult(event.getSubject());
        } else {
            if(PATTERN_DND.matcher(event.getMessage().contentToString()).matches()){
                DiceManager.getCustomDice(captureFromPatternDND(event.getMessage().contentToString()).get(1),
                ...
                DiceManager.getCustomDice(captureFromPatternDNDSingleRoll(event.getMessage().contentToString()),1)
                        .broadcastResult(event.getSubject());
            }
        }
    }
